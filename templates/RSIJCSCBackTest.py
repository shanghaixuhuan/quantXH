...
from strategies.RSIJCSC_backtest import RSIJCSCtest
from templates.BackTestResult import BackTestResult
from PyQt5.QtCore import Qt


class RSIJCSCBackTest(QDialog):
    def __init__(self):
        super(RSIJCSCBackTest,self).__init__()
        self.codepool = ["000001","000002","000004",]
        self.initUI()

    def initUI(self):
        self.resize(900,600)
        self.setWindowTitle("QUANT XH 金融终端——RSI 金叉死叉回测")
        ...
        self.nshortlabel = QLabel()
        self.nshortlabel.setText("短线N:")
        self.nshortlabel.setFont(QFont("仿宋", 15))
        self.nshortedit = QTextEdit()
        self.nshortedit.setText("6")
        self.nshortedit.setFont(QFont("仿宋", 12))
        self.nshortedit.setFixedSize(50, 30)
        self.nlonglabel = QLabel()
        self.nlonglabel.setText("长线N:")
        self.nlonglabel.setFont(QFont("仿宋", 15))
        self.nlongedit = QTextEdit()
        self.nlongedit.setText("12")
        self.nlongedit.setFont(QFont("仿宋", 12))
        self.nlongedit.setFixedSize(50, 30)

        self.texttb = QTextBrowser()
        self.texttb.setFont(QFont("仿宋", 10))
        self.texttb.setFixedSize(300, 150)
        self.texttb.setText("RSI指标的金叉，是指短线RSI线自下而上突破长线RSI线形成黄金交叉，金叉发出买人信号。\n\n"
                            "RSI指标的死叉，即短线RSI线自上而下跌破长线RSI线形成死亡交叉，是卖出信号。其中，出现在RSI指标高位超买区的死叉发出的卖出信号更为强烈。")
        ...
        datefrom = datetime.strptime("2019-07-01", "%Y-%m-%d").date()
        ...
        self.h22box.addWidget(self.nshortlabel)
        self.h22box.addWidget(self.nshortedit)
        self.h22box.addStretch(1)
        self.h22box.addWidget(self.nlonglabel)
        self.h22box.addWidget(self.nlongedit)
        self.h22box.addStretch(1)
    ...
    def backTest(self):
        ...
        r = RSIJCSCtest(cash=int(self.cashedit.toPlainText()))
        r.RSIJCSC_backtest(code=self.codepool,ckstart=change_name(self.ckfromtimeedit.text()),
                        ...
                        Nshort=int(self.nshortedit.toPlainText()),Nlong=int(self.nlongedit.toPlainText()))
        r.save_to_mongo()
...
if __name__ == '__main__':
    ...
    rsijcscbtwindow = RSIJCSCBackTest()
    rsijcscbtwindow.show()
    sys.exit(app.exec_())
